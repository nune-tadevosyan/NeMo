...
def align_sequences(
    ...
    # backtrack
    i, j = n, m
    alignment: List[Tuple[Optional[int], Optional[int], float]] = []
    while i > 0 or j > 0:
        move = bp[i][j]
        if move == 0:  # M
            sim = word_sim(ref_words[i-1], hyp_words[j-1])
...
def create_llm_timestamps_from_alignment(
    ...
    Args:
        alignments: List of alignment results from align_sentences, one per sentence pair. Each alignment
            ...
            hyp_idx is the LLM word index, and either can be None for gaps.
        asr_hyps: List of ASR hypothesis objects, one per sentence. Each object must have a 'timestamp'
    ...
    Returns:
        List of word timestamp lists, one per LLM sentence. Each word timestamp is a dictionary with:
            ...
            - 'start_offset': int - start character offset in the audio
    ...
    Note:
        ...
        2. Unaligned LLM word: Searches for containing word in ASR and splits timestamp proportionally
        3. Unaligned ASR word: Checks if it's part of previous LLM word and updates boundaries
    ...
    def find_containing_word(target_word: str, candidate_words: List[str], candidate_timestamps: List[Dict], similarity_threshold: float = 0.7) -> Optional[Tuple[int, Dict, str, float]]:
        ...
        Returns:
            ...
            match is found above the threshold. The tuple contains:
                - index: int - position of the matching candidate word in candidate_words
                - timestamp: Dict - timestamp dictionary for the matching candidate word
                - candidate_word: str - the matching candidate word
    ...
    def split_timestamp(timestamp: Dict, target_word: str, containing_word: str, is_first_part: bool) -> Dict:
        ...
        Args:
            ...
            target_word: The LLM word that is part of the containing word.
        ...
        if is_first_part:
            ...
            new_end_offset = start_offset + int(offset_duration * ratio)
            return {
                'word': target_word,
                'start': start_time,
                'end': new_end_time,
            ...
            # Second part gets middle to end
            new_start_time = start_time + (duration * (1 - ratio))
            new_start_offset = start_offset + int(offset_duration * (1 - ratio))
            return {
                'word': target_word,
            ...
            if hyp_idx is not None:  # LLM word exists
                ...
                if ref_idx is not None and ref_idx < len(asr_word_timestamps):
                    ...
                    if is_first_part:
                        llm_timestamp = {
                            'word': llm_word,
                            'start': sentence_timestamps[-1].get('end', 0.0),
                            'end': asr_timestamp.get('end', 0.0),
                            'start_offset': sentence_timestamps[-1].get('end_offset', 0.0),
                        ...
                        llm_timestamp = {
                            'word': llm_word,
                            'start': asr_timestamp.get('start', 0.0),
                            'end': asr_timestamp.get('end', 0.0),
                    ...
                    if containing_result is not None:
                        ...
                        # No containing word found - use None timestamps
                        if sentence_timestamps:
                            llm_timestamp = {
                                'word': llm_word,
                                'start': sentence_timestamps[-1].get('end', 0.0),
                                'end': sentence_timestamps[-1].get('end', 0.0),
                                'start_offset': sentence_timestamps[-1].get('end_offset', 0),
                                'end_offset': sentence_timestamps[-1].get('end_offset', 0),
                            }
                        else:
                            llm_timestamp = {
                                'word': llm_word,
