...
def get_utt_obj_tdt(
    ...
    Returns:
        Utterance object with the following fields:
            ...
            - audio_filepath: the path to the audio file if provided.
            - utt_id: the ID of the utterance if provided.
            - token_ids_with_blanks: a list of token IDs without blanks.
    """
    ...
    # build up lists: token_ids_with_blanks (now without blanks), segments_and_tokens.
    if hasattr(model, 'tokenizer'):
        if hasattr(model, 'blank_id'):
            BLANK_ID = model.blank_id
        else:
        ...
        # Initialize without blank tokens
        utt.token_ids_with_blanks = []

        if len(text) == 0:
            # Add blank token for empty utterances
            utt.token_ids_with_blanks = [BLANK_ID]
            ...
            # Create a blank word within the segment
            utt.segments_and_tokens[-1].words_and_tokens.append(
                Word(text="", s_start=0, s_end=0)
            )
            
            # Create a blank token within the word
            utt.segments_and_tokens[-1].words_and_tokens[-1].tokens.append(
                Token(
                    text="<blank>",
                    text_cased="<blank>",
        ...
        # build up data structures containing segments/words/tokens
        segment_s_pointer = 0  # first segment will start at s=0 (no initial blank)
        word_s_pointer = 0     # first word will start at s=0 (no initial blank)
        ...
        for segment in segments:
            ...
            # s_end is the index of the final token of the current segment (no blanks to account for)
            ...
            for word_i, word in enumerate(words):
                ...
                # add the word to word_info and increment the word_s_pointer (no blanks)
                word_s_end = word_s_pointer + len(word_tokens) - 1
                utt.segments_and_tokens[-1].words_and_tokens.append(
                    Word(text=word, s_start=word_s_pointer, s_end=word_s_end)
                )
                word_s_pointer = word_s_end + 1

                for token_i, (token, token_id, token_cased) in enumerate(
                    zip(word_tokens, word_token_ids, word_tokens_cased)
                ):
                    # add the text tokens without blanks
                    ...
                    utt.segments_and_tokens[-1].words_and_tokens[-1].tokens.append(
                        Token(
                            ...
                            text_cased=token_cased,
                            token_id=token_id,
                            # token position in the continuous sequence (no blanks)
                            s_start=len(utt.token_ids_with_blanks) - 1,
                            s_end=len(utt.token_ids_with_blanks) - 1,
                        )
                    )
...
def create_char_timestamps(utt_obj: Utterance, alignment_utt: List[int], output_timestep_duration: float, durations, timesteps, tokenizer, supported_punctuation=['.', ',', '!', '?', ':']):
    """
    Function to add t_start and t_end (representing time in seconds) to the Utterance object utt_obj.
    Args:
        utt_obj: Utterance object to which we will add t_start and t_end for its
    ...
    idx = 0
    count_dict = {x: alignment_utt.count(x) for x in set(alignment_utt)}
    if len(timesteps) == 0:
        # Set all tokens to start and end at 0
        char_timestamps = []
        for segment_or_token in utt_obj.segments_and_tokens:
            if type(segment_or_token) is Segment:
                segment = segment_or_token
                        ...
                        for token in word.tokens:
                            if token.token_id != BLANK_ID:
                                ...
                                char_timestamps.append(
                                    {
                                        "char": tokenizer.ids_to_text([token.token_id]),
                                        "token_id": token.token_id,
                                        "token": token.text,
                                        "start_offset": token.s_start,
                                        "end_offset": token.s_end,
        ...
        return char_timestamps, utt_obj
        ...
        if type(segment_or_token) is Segment:
            ...
            for word_or_token in segment.words_and_tokens:
                if type(word_or_token) is Word:
                    word = word_or_token
                    for token in word.tokens:
                        if token.s_start in count_dict:
                            if  token.token_id != BLANK_ID:
                                try:
                                    start = timesteps[idx]
                                except:
                                    import pdb; pdb.set_trace()
                                token.t_start = start * output_timestep_duration
                                if token.text not in supported_punctuation:
                                    token.s_end = timesteps[idx] + durations[idx]
                                    if count_dict[token.s_start] == 1:
                                        idx += 1
                                    elif count_dict[token.s_start] > 1:
                                        # Sum durations for all appearances of this token
                                        #duration_sum = 0
                                        # for i in range(count_dict[token.s_start]):
                                        #     duration_sum += durations[idx + i]
                                        # token.s_end = token.s_start + duration_sum
                                        # Advance idx by the number of appearances
                                        idx += count_dict[token.s_start]
                                    
                                            
                                elif token.text in supported_punctuation and idx > 0:
                                    token.s_end = timesteps[idx - 1] + durations[idx - 1]
                                    idx += 1
                                token.s_start = start
                                token.t_end = token.s_end * output_timestep_duration
                                
                            else:
                                idx += count_dict[token.s_start]
                        elif token.token_id != BLANK_ID:
                            if idx > 0:
                                # Set start and end to the end of the last timestamp
                                token.s_start = timesteps[idx - 1] + durations[idx - 1]
                                token.s_end = timesteps[idx - 1] + durations[idx - 1]
                                token.t_start = token.s_start * output_timestep_duration
                                token.t_end = token.s_end * output_timestep_duration
                            else:
                                token.s_start = 0
                                token.t_start = 0
...
def get_batch_variables_omni(
    model: ASRModel,
    hypotheses: Union[List[str], str] = None,
    gt_text_batch: Union[List[str], str] = None,
    output_timestep_duration: Optional[float] = None,
    padding_value: float = -3.4e38,
    align_using_pred_text: bool = False,
    ...
    for hypothesis in hypotheses:
        ...
        log_probs_list_batch.append(hypothesis.alignments[0])
        T_list_batch.append(hypothesis.alignments[0].shape[1])
        pred_text_batch.append(hypothesis.text)
